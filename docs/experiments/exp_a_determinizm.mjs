...
const src = readFileSync("./diffuse.wgsl", "utf8");
...
for (let i = 0; i < CELLS; i++) field[i] = 0.45 + rnd() * 0.35;  // вокруг порога 0.6
...
for (let s = 0; s < STEPS; s++) {
  ...
  if (s < 5 || (s + 1) % 50 === 0 || (evDiff > 0 && firstEventDiff === s + 1)) {
    trace.push({ step: s + 1, bitDiff, pct: (100 * bitDiff / CELLS), maxAbs, evDiff });
  }
  if (firstEventDiff > 0 && s + 1 >= firstEventDiff + 20) break;
}

console.log("=".repeat(76));
console.log("ЭКСПЕРИМЕНТ A — расходятся ли ИСТОРИИ от порядка сложения (Dawn/llvmpipe)");
console.log("=".repeat(76));
